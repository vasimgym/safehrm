...
if (!empty($_GET["category"])) {
	...
	if (!empty($subcategory)) {
		$extrasql = " WHERE s_category='$category' AND s_subcategory REGEXP($subcategory) AND s_status='EXPIRED'";
	} else {
		$extrasql = " WHERE s_category='$category' AND s_status='EXPIRED'";
	}
	
} else {
	$extrasql = " WHERE s_status='EXPIRED'";
}
...
if(!empty($_GET["category"])) {
	...
	$subcategory = $_GET["subcategory"];
	if (!empty($subcategory)) {
		$sql .= " WHERE s_category='$category' AND s_subcategory REGEXP($subcategory) AND s_status='EXPIRED'";
	} else {
		$sql .= " WHERE s_category='$category'  AND s_status='EXPIRED'";
	}
	
} else {
	$sql .= " WHERE  s_status='EXPIRED'";
}
								...
								<script src="../js/functions.js"></script>
								  <form id="form1" name="form1" method="get" action="" >
								    <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                      <tbody>
                                        <tr>
                                          <td class="orange" height="16">Manage expired assignments </td>
                                        </tr>
                                        <tr>
